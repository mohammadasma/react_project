...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        if (storedToken) {
            setToken(true); // now show profile icon
        }
    }, []);
            ...
            <div className="flex items-center gap-4">
                {
                    token
                        ? <div className="flex-items-center gap-2 cursor-pointer group relative">
                            ...
                            <img className="w-2.5 float-right " src={assets.dropdown_icon} alt="" />
                            <div className="absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block">
                                <div className="min-w-48 bg-stone-100 rounded flex flex-col gap-5">
                ...
                <div className={` ${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                    ...
                    <ul className="flex flex-col items-center gap-2 mt-5 text-lg font-medium">
                        <NavLink className="px-4 py-2 rounded  inline-block" onClick={() => { setshowMenu(false); scrollTo(0, 0) }} to="/"><p className="px-4 py-2 rounded inline-block">HOME</p></NavLink>
